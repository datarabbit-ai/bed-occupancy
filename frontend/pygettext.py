...
def escape_ascii(s, encoding):
    return "".join(escapes[ord(c)] if ord(c) < 128 else c if c.isprintable() else escape_nonascii(c, encoding) for c in s)
...
class GettextVisitor(ast.NodeVisitor):
    ...
    def _extract_message(self, node):
        ...
        if len(errors) == 1:
            ...
            print(f'*** {self.filename}:{node.lineno}: No keywords matched gettext call "{func_name}":', file=sys.stderr)
    ...
    def _extract_message_with_spec(self, node, spec):
        ...
        if max_index >= len(node.args):
            return f"Expected at least {max_index + 1} positional argument(s) in gettext call, got {len(node.args)}"

        msg_data = {}
        for arg_type, position in spec.items():
            arg = node.args[position]
            if not self._is_string_const(arg):
                return f"Expected a string constant for argument {position + 1}, got {ast.unparse(arg)}"
